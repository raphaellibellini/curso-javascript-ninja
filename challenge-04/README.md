...
carro.addPessoas = function(numPessoas) {
  let assentosVagos = carro.assentos - carro.quantidadePessoas;
  if(carro.quantidadePessoas < carro.assentos) {
    if(numPessoas > assentosVagos){
      let singularPluralPessoa = assentosVagos === 1 ? 'pessoa' : 'pessoas';
      return `Só cabem mais ${assentosVagos} ${singularPluralPessoa}!`
    }
  }
...
carro.obterMarcaModelo(); // 'Esse carro é um Citroen C3'
